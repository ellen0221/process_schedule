...
public class OS_process extends JFrame{
    ...
    public OS_process() {
        ...
        RR_button.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                int tslice = Integer.parseInt(timeslice.getText());
                List<Process> list = new ArrayList<>();
                list.addAll(ListOfProcess);
                List<Process> finish = arithmetic.RR(list, tslice);
                for (int i = 0; i < finish.size(); i++) {
                    Process p = finish.get(i);
                    switch (p.getName()) {
                        case "A":
